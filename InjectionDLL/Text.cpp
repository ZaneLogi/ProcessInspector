...
bool Text::initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight)
{
    ...
    if (!m_Font)
    {
        LOG << "Failed to allocate BitmapFont\n";
        return false;
    }
    ...
    if (!result)
    {
        LOG << "Could not initialize the font object.\n";
        return false;
    }
    ...
    if (!m_FontShader)
    {
        LOG << "Failed to allocate BitmapFontShader\n";
        return false;
    }
    ...
    if (!result)
    {
        LOG << "Could not initialize the font shader object.\n";
        return false;
    }
    ...
    if (!result)
    {
        LOG << "Failed to call InitializeSentence(1)\n";
        return false;
    }

    // Now update the sentence vertex buffer with the new string information.
    result = UpdateSentence(m_sentence1, "InjectDll.", 0, 0, 1.0f, 1.0f, 1.0f, deviceContext);
    if (!result)
    {
        LOG << "Failed to call UpdateSentence(1)\n";
        return false;
    }
    ...
    if (!result)
    {
        LOG << "Failed to call InitializeSentence(2)\n";
        return false;
    }

    // Now update the sentence vertex buffer with the new string information.
    result = UpdateSentence(m_sentence2, "Horay!", 0, 24, 1.0f, 1.0f, 0.0f, deviceContext);
    if (!result)
    {
        LOG << "Failed to call UpdateSentence(2)\n";
        return false;
    }
    ...
    const XMMATRIX& orthoMatrix)
{
    ...
    result = m_FontShader->render(deviceContext, sentence->indexCount, orthoMatrix, m_Font->get_texture(),
        pixelColor);
    if (!result)
    {
        LOG << "Failed to call BitmapFontShader::render\n";
        false;
    }
