...
BEGIN_MESSAGE_MAP(CTesterDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
    ON_BN_CLICKED(IDC_BTN_GET, &CTesterDlg::OnBnClickedBtnGet)
    ON_BN_CLICKED(IDC_BTN_SET, &CTesterDlg::OnBnClickedBtnSet)
END_MESSAGE_MAP()
...
void CTesterDlg::OnBnClickedBtnGet()
{
    SetDlgItemTextA(GetSafeHwnd(), IDC_EDIT1, m_strTest);
}


void CTesterDlg::OnBnClickedBtnSet()
{
    GetDlgItemTextA(GetSafeHwnd(), IDC_EDIT1, m_strTest, 64);
}
